...
public class Form1 : Form
{
    ...
    private async void ReadMessageAsync(TcpClient client)
    {
        ...
        else if (message.Contains("Middleware 2"))
        {
            ...
            localTimestamp.Add(new List<int> { firstTimestamp, 2 });
            richTextBox2.AppendText($"{message}\n");
            receivedMessage.Add(message);
            SendTimestamp(holdingTimestamp);
        }
    ...
    private async void SendTimestamp(List<List<int>> holdingTimestamp)
    {
        if (holdingTimestamp.Count >= 1)
        {
            if (holdingTimestamp[holdingTimestamp.Count - 1][1] == 1)
            {
                ...
                await clientToM1.GetStream().WriteAsync(data, 0, data.Length);
            }
    ...
    private async void button1_Click(object sender, EventArgs e)
    {
        ...
        byte[] data = Encoding.UTF8.GetBytes("Msg #" + index + " from Middleware 2");
    ...
    public async void ReceiveTimestampMessageAndSendFinalTs(int timestamp, int source)
    {
        ...
        if (timestampMessageCount == TotalMiddlewareCount)
        {
            ...
            TcpClient clientToM1 = new TcpClient("localhost", 8082);
            byte[] data1 = Encoding.UTF8.GetBytes("FTS: " + finalLocalTimestamp + " From: " + 2);
            await clientToM1.GetStream().WriteAsync(data1, 0, data1.Length);
