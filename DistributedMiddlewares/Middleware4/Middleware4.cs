...
public class Form1 : Form
{
    ...
    private async void ReadMessageAsync(TcpClient client)
    {
        ...
        else if (message.Contains("Middleware 4"))
        {
            ...
            localTimestamp.Add(new List<int> { firstTimestamp, 4 });
            richTextBox2.AppendText($"{message}\n");
            receivedMessage.Add(message);
            SendTimestamp(holdingTimestamp);
        }
        ...
        if (holdingTimestamp.Count >= 1)
        {
            ...
            else if (holdingTimestamp[holdingTimestamp.Count - 1][1] == 4)
            {
                ...
                if (timestampMessageCount == TotalMiddlewareCount)
                {
                    ...
                    byte[] data1 = Encoding.UTF8.GetBytes("FTS: " + finalLocalTimestamp + " From: " + 4);//FTS means final timestamp
                    await clientToM1.GetStream().WriteAsync(data1, 0, data1.Length);

                    TcpClient clientToM2 = new TcpClient("localhost", 8083);
                    byte[] data2 = Encoding.UTF8.GetBytes("FTS: " + finalLocalTimestamp + " From: " + 4);
                    await clientToM2.GetStream().WriteAsync(data2, 0, data2.Length);

                    TcpClient clientToM3 = new TcpClient("localhost", 8084);
                    byte[] data3 = Encoding.UTF8.GetBytes("FTS: " + finalLocalTimestamp + " From: " + 4);
                    await clientToM3.GetStream().WriteAsync(data3, 0, data3.Length);

                    TcpClient clientToM5 = new TcpClient("localhost", 8086);
                    byte[] data5 = Encoding.UTF8.GetBytes("FTS: " + finalLocalTimestamp + " From: " + 4);
    ...
    private async void button1_Click(object sender, EventArgs e)
    {
        ...
        byte[] data = Encoding.UTF8.GetBytes("Msg #" + index + " from Middleware 4");
    ...
    public async void ReceiveTimestampMessageAndSendFinalTs(int timestamp, int source)
    {
        ...
        if (timestampMessageCount == TotalMiddlewareCount)
        {
            ...
            //sent final local timestamp
            TcpClient clientToM1 = new TcpClient("localhost", 8082);
            byte[] data1 = Encoding.UTF8.GetBytes("FTS: " + finalLocalTimestamp + " From: " + 4);
            await clientToM1.GetStream().WriteAsync(data1, 0, data1.Length);
