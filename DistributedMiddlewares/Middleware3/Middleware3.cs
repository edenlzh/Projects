...
public class Form1 : Form
{
    ...
    private async void ReadMessageAsync(TcpClient client)
    {
        ...
        else if (message.Contains("Middleware 3"))
        {
            ...
            localTimestamp.Add(new List<int> { firstTimestamp, 3 });
            richTextBox2.AppendText($"{message}\n");
            receivedMessage.Add(message);
            SendTimestamp(holdingTimestamp);
        }
    ...
    private async void SendTimestamp(List<List<int>> holdingTimestamp)
    {
        if (holdingTimestamp.Count >= 1)
        {
            ...
            {
                TcpClient clientToM2 = new TcpClient("localhost", 8082);
            ...
            else if (holdingTimestamp[holdingTimestamp.Count - 1][1] == 3)
            {
                ...
                if (timestampMessageCount == TotalMiddlewareCount)
                {
                    ...
                    TcpClient clientToM1 = new TcpClient("localhost", 8082);
                    byte[] data1 = Encoding.UTF8.GetBytes("FTS: " + finalLocalTimestamp + " From: " + 3);//FTS means final timestamp
                    await clientToM1.GetStream().WriteAsync(data1, 0, data1.Length);

                    TcpClient clientToM2 = new TcpClient("localhost", 8083);
                    byte[] data2 = Encoding.UTF8.GetBytes("FTS: " + finalLocalTimestamp + " From: " + 3);
                    await clientToM2.GetStream().WriteAsync(data2, 0, data2.Length);

                    TcpClient clientToM4 = new TcpClient("localhost", 8085);
                    byte[] data4 = Encoding.UTF8.GetBytes("FTS: " + finalLocalTimestamp + " From: " + 3);
                    await clientToM4.GetStream().WriteAsync(data4, 0, data4.Length);

                    TcpClient clientToM5 = new TcpClient("localhost", 8086);
                    byte[] data5 = Encoding.UTF8.GetBytes("FTS: " + finalLocalTimestamp + " From: " + 3);
    ...
    private async void button1_Click(object sender, EventArgs e)
    {
        ...
        byte[] data = Encoding.UTF8.GetBytes("Msg #" + index + " from Middleware 3");
        await client.GetStream().WriteAsync(data, 0, data.Length);
        richTextBox1.AppendText("Message " + index + " has been sent.\n");
        index += 1;
    }

    public async void ExtractTimestampAndSource(string message)
    {
    ...
    public async void ReceiveTimestampMessageAndSendFinalTs(int timestamp, int source)
    {
        ...
        if (timestampMessageCount == TotalMiddlewareCount)
        {
            ...
            TcpClient clientToM1 = new TcpClient("localhost", 8082);
            byte[] data1 = Encoding.UTF8.GetBytes("FTS: " + finalLocalTimestamp + " From: " + 3);
            await clientToM1.GetStream().WriteAsync(data1, 0, data1.Length);
    ...
    public int GetMaxLocalTimestampAndClear(List<List<int>> localTimestamp)
    {
    ...
    public void ExtractFinalTimestampAndSource(string message)
    {
    ...
    public void PrintReadyMessage(List<List<int>> finalTimestamps, List<string> receivedMessage)
    {
